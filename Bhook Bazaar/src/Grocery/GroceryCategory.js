...
const GroceryCategory = () => {
    ...
    return (
        ...
        <span className="lg:text-xl text-base sm:text-lg  md:text-xl font-extrabold px-2 text-gray-950">{groceryTitle}</span>
            <div className="flex lg:justify-between justify-evenly sm:justify-between md:justify-between items-start  flex-row flex-wrap pt-6 gap-4 lg:gap-6 md:gap-4">
               ...
               
                {
                    groceryList?.map((gros) => 
                        <div className="flex justify-center items-center lg:w-[100px] md:w-[80px] gap-2 flex-col">
                            <img className="lg:w-24 md:w-24 w-20" src={CARD_IMG_URL + gros.imageId}></img>
                           {gros.imageId && <div className="flex justify-center items-center ">
                            <span className="text-xs text-center hidden lg:flex font-bold text-gray-700">
                                {gros.displayName.length > 25 ?  `${gros.displayName.slice(0,25)}...` : gros.displayName}</span>
                                <span className="text-xs text-center flex lg:hidden font-bold text-gray-700">
                                {gros.displayName.length > 10 ?  `${gros.displayName.slice(0,10)}...` : gros.displayName}</span>
                            </div>}
                        </div>
                    )
                }
